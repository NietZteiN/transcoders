...
- [x] **H-R23 (ASE) — is there ANY permitted host with a renaming deficit? (raised by H-R22, run 2026-09-18).** Pre-registered in `log/nla-harness/2026-09-18_panel-damage-prereg.md`, results in `log/nla-harness/2026-09-18_panel-damage-results.md`. L0 vs the H-R22 derangement under greedy on the 146 PACKS-PAIRED snippets, `unsteered` only (no steering, no vectors, no alignment), for every permitted instruct model in the offline cache. All four passed the chat-template ids gate **with their own native format** (Gemma `<bos><start_of_turn>`, Llama-3.1 `<|begin_of_text|>`, Phi `<|user|>`, CodeLlama-13b `<s>[INST]`), so no `PROTOCOL-BROKEN` exclusions. **RESOLVED `NO-PERMITTED-HOST`, as predicted.** `codegemma-7b-it` **+0.0070** [−0.023, +0.037] — **its H-R1 +0.076 does not survive** (~10×; H-R1 used the weak renamer, sampled decoding and a different harness, all shown by H-R15/H-R18 to move numbers by more than the effect), and it was the model flagged most likely to be a host · `Llama-3.1-8B-Instruct` **+0.0168** [−0.017, +0.052], the largest readable damage, CI containing 0 · `CodeLlama-13b-Instruct` **−0.0244** — the deranged code scores *higher*, and its L0 **0.7631** is **below** the 7B's 0.8531, so **the null is neither a scale nor a family artefact** · reference CodeLlama-7B +0.0122; their Qwen2.5-7B **+0.3629**. **The pre-registered floor gate is the most valuable line in the entry:** `Phi-3.5-mini-instruct` returned **+0.0546 with a CI excluding zero** — formally the only `DAMAGE-SUFFICIENT` cell in the whole block, which would have been reported as `HOST-FOUND` and moved the steering programme there — but its **L0 accuracy is 0.5639 against chance 0.500** with parse **0.768**, so it is degrading from near-chance to chance and its damage is **not read**. Per-item agreement **0.76–0.90 panel-wide**, so H-R22's "these models trace code rather than read identifier names" is a **panel property**. Whole panel cost **~45 min GPU**. Raises **H-R24** (a scope call) and **H-R25** (is Phi-3.5-mini a capability floor or a formatting failure — free).
- [x] **H-R22 (ASE) — an adversarial stimulus that actually damages (raised by H-R18/H-R21, run 2026-09-18).** Pre-registered in `log/nla-harness/2026-09-18_adversarial-rename-prereg.md`, results in `log/nla-harness/2026-09-18_adversarial-rename-results.md`. Diagnosed two faults in the renamer behind **every** banked renaming number: `len(n) > 1` skipped **all single-character identifiers** (most loop counters and accumulators), and decoys were assigned by `rng.sample` **at random** — making the banked stimulus closer to this project's **L1 (nonsense)** than **L1b (adversarial)**. Replaced by a seeded **derangement** of each file's own declared identifiers (Sattolo, no fixed point), including single-char names and the method name: every identifier keeps a name plausible *in that program* while attached to the wrong entity, and the **set of names is exactly preserved**, so damage cannot be attributed to unfamiliar tokens. **RESOLVED.** **a `CORPUS-OK`**: 148/148 deranged, **837 identifiers renamed of which 365 single-char** (the old renamer renamed zero), **0 compile or execution failures** (the empirical proof that a derangement is semantics-preserving), **146/148** PACKS-PAIRED, 11.73 cases/snippet vs their 11.72. **b `DAMAGE-PRESENT-BUT-SMALL`, refuting my `DAMAGE-SUFFICIENT` prediction**: L0 **0.8531** → derangement **0.8409**, damage **+0.0122 [−0.0125, +0.0382]**, harder than the weak renamer by only **+0.0052 [−0.022, +0.035]**, and **3.4 %** of their Qwen2.5-7B +0.3629. **The diagnostic is the substantive result:** per-item agreement with the original is **0.9286**, only **7.1 %** of labels flip and the churn is near-symmetric (keeps 95.1 % of L0-correct, rescues 20.2 % of L0-wrong) — **CodeLlama-7B-Instruct traces the code and essentially ignores identifier semantics**, independently reproducing H-R18's oracle ceiling from an unrelated measurement and explaining every null in this block at once. Parse **1.0000** in all three conditions. Pre-commitment honoured: **renaming is CLOSED as a stimulus for this model** and no steering arm will run on one. Raises **H-R23**.
- [x] **H-R18 (ASE) — the greedy pass (raised by H-R15, run 2026-09-17/18).** Pre-registered in `log/nla-harness/2026-09-17_greedy-pass-prereg.md`, results in `log/nla-harness/2026-09-18_greedy-pass-results.md`. All 8 arms on the frozen 148 snippets with argmax instead of their T 0.7 sampler (`do_sample=False` on **their** `run_llama`), one run per case — H-R15b's exact reproducibility makes the snippet bootstrap the *complete* uncertainty rather than a lower bound. **Declared deviation:** their Pass@k presupposes sampling, so this complements and does not replace the sampled pass; H-R14's numbers stand. **RESOLVED.** **a `GREEDY-EXACT`** (agreement **1.000000**, |Δacc| 0). **b all five H-R14 verdict words hold unchanged** under the *same frozen* thresholds — `DAMAGE-ABSENT` +0.0069 · `CODESTEER-INERT` · `NLA-MATCHES-CODESTEER` **+0.0103** (sign flipped from −0.0409) · `PROMPT-SUFFICES` +0.0098 · `DAMAGE-FAR-WEAKER` ratio **0.019** · restoration withheld — with CIs **1.7–3.9× tighter** (`swap_oracle` ±0.0466 → **±0.0121**). **c** argmax lifts every arm **+0.111…+0.164**, so **the sampler was costing ~16 accuracy points** — the largest effect in this entire block (→ **H-R20**). **The decisive numbers:** renaming damage **+0.0069** and the ORACLE ceiling **+0.0052 [−0.007, +0.017]** — writing the true clean state at L7 buys half a point, so **L7 latent steering is retired on this model** and **this corpus can no longer support a steering claim of any kind** (→ **H-R21**, which supersedes the CruxEval arms). `codesteer` is **−0.0270 [−0.0519, −0.0073]**, the only interval in either pass excluding zero: a small but reliable ~2.7-point **harm** (the frozen ±0.05 magnitude rule still returns `INERT` and was **not** rewritten). Parse is 1.000 for 7 of 8 arms, so the whole parse/compliance story was the sampler and **H-R16 stays void**; `prompt` and `codesteer_auto` both flip sign, confirming H-R15 from the other direction. Also fixed: a phantom-key parse metric this run exposed (`parsed_frac` counted invented case ids, giving parse > 1.0 under greedy) — accuracy never affected, H-R14 re-scored **byte-identical**, no published number moves.
- [x] **H-R15 (ASE) — the replication floor (raised by H-R14, run 2026-09-17).** Pre-registered in `log/nla-harness/2026-09-17_replication-floor-prereg.md`, results in `log/nla-harness/2026-09-17_replication-floor-results.md`. Five replicates on the frozen 148 snippets, same packs / runtime / vectors, varying **only** `--seed` (new flag, defaults to the frozen constant so nothing banked changes) — plus a **same-seed control**, because every run in this family already shared one hard-coded seed and the "seed noise" framing in H-R14 was therefore wrong. **RESOLVED, and it removes the last standing effect.** **b `SEED-DETERMINISTIC`** (my prediction ✗): same seed reproduces **exactly** — per-item agreement **1.0000**, |Δacc| = **0.000000**, across different nodes — so the H-R7→H-R14 drift was **RNG-stream position** (148 snippets reach each snippet at a different point in the stream than 50), not hardware and not a seed. **a `CI-OPTIMISTIC`** ✓: contrast drift **0.0191 (0.46×)** for `ridge_map` and **0.0409 (1.08×)** for `codesteer_auto` against their published half-widths; per-run decoding σ = **0.0092**, so decoding-inclusive intervals are **≈1.2× wider** (`ridge_map` ±0.0414→±0.0486, `codesteer_auto` ±0.0379→±0.0456). **c `AUTO-DOES-NOT-REPLICATE`** (my prediction ✗): `codesteer_auto − unsteered` **+0.0379 → −0.0031** — the only arm whose CI excluded zero in H-R14 is gone, and its parse-compliance component with it (Δparse +0.0293 → −0.0019), which **voids H-R16**. A different draw flips **38 %** of individual case labels (cross-seed agreement 0.611–0.630) while aggregate accuracy moves only σ = 0.0092 — **aggregate stability hides item-level chaos** (→ H-R19). Pre-committed consequence honoured: **no steering claim in this programme may rest on a single run**, and the H-R14 report and artifact are amended. **Survives untouched:** the oracle bound (+0.0163), an upper bound rather than a contrast, so "no better L7 vector can beat CodeSteer" still holds. Raises **H-R18** (greedy decoding pass, ~3 GPU-h — exact determinism makes it a free precision win) and **H-R19**.
...
- [x] **H-R3 (ASE) ✗ CLOSED 2026-09-18 by H-R22** — it asked whether a stronger renamer (all declared identifiers incl. single-char and fields) closes the damage gap to their Qwen2.5-7B. The strongest renaming available — a vocabulary-preserving derangement of *everything*, method name included, 837 identifiers with 365 single-char — reaches **+0.0122**, i.e. **3.4 %** of their +0.3629. It does not, and the reason is that the model is not reading identifier names at all (per-item agreement 0.9286). Superseded by **H-R23** (is there any permitted host with a renaming deficit?).
...
## Changelog
- **2026-09-18c** — **H-R23 (ASE) added and closed**: the panel damage screen returns `NO-PERMITTED-HOST` across four models 3.8B–13B. codegemma's H-R1 damage does not survive; CodeLlama-13b shows negative damage; the floor gate correctly withheld Phi-3.5-mini's formally-sufficient +0.0546 at L0 0.5639. **H-R17 subsumed.** The ASE replication is closed at its honest boundary; **H-R24 is a scope call for the human** (L2/L3 flattening build vs writing up the bounded negative) and **H-R25** is free.
- **2026-09-18b** — **H-R22 (ASE) added and closed**: the identifier derangement. `CORPUS-OK`; `DAMAGE-PRESENT-BUT-SMALL` +0.0122 refutes my prediction; per-item agreement 0.9286 shows the model ignores identifier semantics, which reproduces H-R18's oracle ceiling independently. **Renaming closed as a stimulus**, **H-R3 closed**, **H-R21 resolved** toward another host; **H-R23** raised (panel damage screen).
- **2026-09-18a** — **H-R18 (ASE) added and closed**: the greedy pass. `GREEDY-EXACT`; all five H-R14 words hold with CIs 1.7–3.9× tighter; ~16-point decoder gain (→ H-R20); damage +0.0069 and oracle ceiling +0.0052 retire L7 steering on this model (→ H-R21, superseding CruxEval); `codesteer` a reliable −0.027 harm. Phantom-key parse metric fixed, H-R14 re-scored byte-identical.
- **2026-09-17a** — **H-R15 (ASE) added and closed the same day**: the replication floor. `SEED-DETERMINISTIC` (exact reproduction at a fixed seed) but `CI-OPTIMISTIC` and `AUTO-DOES-NOT-REPLICATE` — H-R14's last surviving effect vanishes at a different draw, taking the parse-compliance story and **H-R16** with it. Intervals ≈1.2× too narrow; 38 % per-item label instability. New: **H-R18** (greedy pass) and **H-R19** (per-item stability). H-R14's report and artifact amended.
