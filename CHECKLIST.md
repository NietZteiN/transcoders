...
- [ ] **H-R10 (ASE) — `role_proto − foreign` as a pre-registered primary on damage-bearing stimuli.** The one oracle-free contrast that has come close: **+0.0776 [−0.0016, +0.152]** against a 0.05 bar needing CI > 0, missed by 0.0016, and both arms write at near-total parse rates (0.998 vs 0.962) so parse-rate inflation is partly differenced out. **Not re-readable on this corpus** — re-reading it here would be retuning after seeing data. To be pre-registered as the primary contrast on H-R7's corpus, same 0.05 bar, before that corpus is scored. Blocked on H-R7.
