#!/usr/bin/env bash
# B3 deception capture (C0/C1/C2 Java variants) — run inside detached tmux:
#   tmux new-session -d -s nla-deception 'bash nla/scripts/deception.sh'
# Smoke: bash nla/scripts/deception.sh --limit 2
#
# Same lifecycle contract as overnight.sh: 2-idle-GPU check (shared box — abort rather than
# stomp another job), AV server on GPU_B with a trap-kill on exit, health wait, runner on GPU_A.
set -euo pipefail
cd "$(dirname "$0")/.."          # nla/
...
PORT=30002                        # not 30000 — avoid colliding with a stray overnight server
...
# AV server in its OWN session (setsid) so cleanup kills the whole tree by pgid without
# touching the driver's group — sglang forks scheduler children.
...
for i in $(seq 1 48); do
  ...
  if ! kill -0 "$SERVER_PID" 2>/dev/null; then
    echo "[driver] ABORT: server died during startup — see $OUT_DIR/server.log"; exit 3
  fi
...
CUDA_VISIBLE_DEVICES="$GPU_A" NLA_SERVER_GPU="$GPU_B" \
  python -m src.deception_capture --sglang-url "http://localhost:$PORT" "$@" \
  2>&1 | tee -a "$OUT_DIR/runner.log"
CODE=${PIPESTATUS[0]}
set -e
echo "[driver] runner exited $CODE"
exit "$CODE"
